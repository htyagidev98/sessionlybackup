...
exports.StudentsList = async (req, res) => {
    try {
        const studentslist = await Payment.find({ teacher_id: req.user._id }).populate([
            {
        ...
        if (studentslist && studentslist.length > 0) {
            ...
            for (let i = 0; i < studentslist.length; i++) {
                const studentslists = studentslist[i];
                let studentObj = {
                    _id: studentslists._id,
                    course: studentslists.course_id,
                    coupon: studentslists.coupon_id,
                    student: studentslists.student,
                    teacher: studentslists.teacher_id,
                    createdAt: moment(studentslists.createdAt).format("DD-MM-YYYY h:mm:ss A"),
                    updatedAt: moment(studentslists.updatedAt).format("DD-MM-YYYY h:mm:ss A"),
                };
                studentslistData.push(studentObj)
            }
            res.status(200).json({
                status: "success",
                counts: studentslist.length,
                responseMessage: "Successfully",
                responseData: studentslistData,
            });
        } else {
    ...
    try {
        ...
        if (paymentList && paymentList.length > 0) {
            ...
            for (let i = 0; i < paymentList.length; i++) {
                ...
                paymentListData.push(paymentObj)
            }
            res.status(200).json({
                status: "success",
...
exports.teacherBookAppointment = async (req, res) => {
    try {
        const appointment = await Appointment.find({ teacher: req.user._id }).populate([
            { path: "student", select: "first_name last_name email role phone" },
